...
use starknet_api::block::{
    BlockHash, BlockInfo, BlockNumber, BlockTimestamp, GasPrice, GasPricePerToken,
    NonzeroGasPrice,
};
use starknet_api::core::{
    ClassHash, ContractAddress, EventCommitment, GlobalRoot, ReceiptCommitment,
...
impl TryInto<BlockInfo> for BlockHeader {
    ...
    fn try_into(self) -> Result<BlockInfo, Self::Error> {
        Ok(BlockInfo {
            ...
            starknet_version: self.starknet_version.try_into().map_err(|err| format!("{err}"))?,
        })
    }
}

fn parse_gas_price(gas_price: GasPrice) -> Result<NonzeroGasPrice, String> {
    NonzeroGasPrice::new(gas_price).map_err(|_| format!("invalid gas price: {gas_price}"))
}
